...
pub fn get_mst_points(points: &[Point]) -> GeometricGraph {
    let g = crate::graph::delaunay::delaunay_points(points);
    g.get_mst()
}
...
mod tests {
            ...
            let mean_diameter = (0..tries)
                .map(|_| {
                    let points = (0..n)
                        ...
                        .collect::<Vec<_>>();
                    let mst = get_mst_points(&points);
                    mst.graph.get_hop_diameter_approx()
                })
                .sum::<usize>() as f64
                / tries as f64;
    ...
    fn kruskal_2d_diam() {
        let out = (5..22)
            ...
            .map(|i| {
                let i = 2usize.pow(i);
                let g = get_mst(i);
                let diam = g.graph.get_hop_diameter_approx();
                format!("{} {}\n", i, diam)
            })
            .collect::<String>();
    ...
    fn simple_kruskal_3d() {
        for i in [10_000, 20_000, 50_000, 70_000, 100_000, 500_000, 1_000_000] {
            let g = kruskal3d(i);
            println!("{} {}", i, g.0.get_hop_diameter_approx());
        }
    }
    ...
    fn diam_kruskal_3d() {
        ...
        let diams = [10_000, 20_000, 40_000, 80_000, 160_000, 320_000]
            ...
            .map(|i| {
                let g = kruskal3d(i).0;
                let diam = g.get_hop_diameter_approx();
                format!("{} {}\n", i, diam)
            })
            .collect::<String>();
